...
| ./data/calibration_matrixes/* | Contains perspective transformation matrices ("perspective_transform.npy", "T_bc.npy") and lens correction matrices ("distortion.npy", "mtx.npy") obtained during camera calibration, features of the healthy dataset pieces ("feature_stats.json") used for statistical filtering, and data scalers ("scaler_X.pkl", "scaler_Y.pkl") for data normalization and scaling used in the Multilayer Perceptron. |
...
3.- **Modelling**: A segmentation model is trained using U-Net [U-Net Model](https://github.com/zhixuhao/unet.git) architecture. A Region of Interest (ROI) filtering block with HSV filtering and contour analysis is applied, followed by a final filtering block comparing features to those in a healthy (non-corrupted) dataset. The (x,y,z) positions of pieces in the working range are transformed to the robot frame using either transformation matrices from the calibration process or an end-to-end MLP model.
...
| File Name | Content |
|-----------|---------|
| main_threaded.py | For the deployment of the model, replace this file in the "Robot Demonstrator/scripts/main_threaded.py" directory of the Matthias de Ryck GitHub repository. Ensure to copy the contents of "data/calibration_matrixes" and "data/models" into the corresponding paths within the repository. |
| Deployment_video | A video showcasing the system in operation. [See Demonstration Video](results/Deployment_video.mp4) |
